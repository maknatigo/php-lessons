...
function renderPage($db, $variables) {
    ...
    if($variables['unit'] == 'products' && isset($_SESSION['auth']) && $_SESSION['auth'] == 1 && isset($_SESSION['role']) && $_SESSION['role'] == 'administrator') {
        $column = file_get_contents('./templates/' . $variables['unit'] . '/column_admin.php');
    } else {
    ...
    $values[] = $result;
    $params[] = '{{CART}}';
    if(isset($_COOKIE['cntItems']) && (int)$_COOKIE['cntItems'] == 0) {
